...
public class SK4StopWatch {
	...
	/// 経過時間を秒単位で取得
	public func totalSecond() -> TimeInterval {
	...
	/// インターバルを秒単位で取得
	public func intervalSecond() -> TimeInterval {
		let now = mach_absolute_time()
	...
	/// インターバルの平均を秒単位で取得
	public func averageSecond(count: Int) -> TimeInterval {
	...
	/// 絶対時間の時間差をナノ秒で求める
	func absTimeDiff(now: UInt64, old: UInt64) -> UInt64 {
		return (now - old) * UInt64(g_timebase.numer) / UInt64(g_timebase.denom)
	}

	/// ナノ秒を秒に変換
	func nanoToSec(_ nano: UInt64) -> TimeInterval {
		return TimeInterval(nano) / TimeInterval(NSEC_PER_SEC)
	}
